...
- `scripts/notify_kr_watchlist.py` — `watchlist.json`의 종목마다 Claude(Sonnet, 2026-09-07 Haiku에서
  ...
  (월요일 실행분은 주말 공백을 메우기 위해 72시간)이내 뉴스를 '사실|해석|판정|URL' 4필드(한국어 강제, 외신은 번역)로
  확인한 뒤, 뉴스가 있는 종목만 모아 **list 카드 2~3개씩 묶어 여러 메시지로** 발송한다(카드 아이콘은
  호재/부정/중립별 색상 아이콘 `assets/icon-*.png`; 뉴스 없는 종목은 언급 안 함). 모델의 원문 응답을 항상 stderr(Actions 로그)에 남겨(`_log_raw`) "뉴스가 진짜 없었는지 vs
