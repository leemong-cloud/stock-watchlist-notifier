...
For each stock, asks Claude (Haiku + the hosted web_search tool) for the most important recent
news and market reaction, in a KakaoTalk-message-sized Korean summary. No numeric claim is
accepted without the model having actually searched for it -- the prompt explicitly tells the
model to say "특이 뉴스 없음" rather than guess.
...
def summarize_stock(client: Anthropic, symbol: str, name: str) -> str:
    prompt = (
    ...
    return "\n".join(text_blocks).strip() or "특이 뉴스 없음"
...
def main() -> int:
    ...
    for i, stock in enumerate(stocks):
        ...
        try:
            summary = summarize_stock(client, symbol, name)
        ...
        except Exception as exc:  # noqa: BLE001 -- keep the loop going for remaining stocks
